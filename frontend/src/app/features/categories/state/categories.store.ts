...
export class CategoriesStore {
  ...
  private resolveErrorMessage(err: unknown, fallback: string): string {
    ...
    if (err instanceof CategoryUnauthorizedError) {
      return 'Your session has expired. Please sign in again.';
    }

    if (err instanceof CategoryForbiddenError) {
    ...
    if (err instanceof CategoryAlreadyExistsError) {
      return err.message || 'A category with this name already exists.';
    }

    if (err instanceof CategoryHasProductsError) {
      return err.message || 'Cannot delete category with associated products.';
    }
