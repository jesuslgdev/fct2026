...
class MockGetSupplierProductsUseCase {
  execute = vi.fn<(supplierId: number, params?: SupplierProductQueryParams) => Observable<PagedResult<SupplierProduct>>>();
}

class MockAddProductToSupplierUseCase {
  execute = vi.fn<(supplierId: number, request: { productId: number; supplierPrice: number }) => Observable<SupplierProduct>>();
}
...
describe('SupplierProductsStore', () => {
  ...
  describe('adding products', () => {
    it('should validate that a product is selected in addSelectedProductToSupplier', async () => {
      ...
      expect(store.error()).toBe('Producto seleccionado invalido.');
    ...
    it('should reject price with more than two decimals', async () => {
    ...
    it('should translate duplicate errors when adding', async () => {
      store.supplierId.set(5);
      addProductToSupplierUseCase.execute.mockReturnValue(throwError(() => new SupplierProductDuplicateError()));

      await store.addProductToSupplier({ productId: 3, supplierPrice: '10' });
  ...
  describe('API and permission errors', () => {
    it('should use API error message when loading active products', async () => {
      ...
      expect(store.error()).toBe('API caida');
      expect(store.productsLoading()).toBe(false);
    });
