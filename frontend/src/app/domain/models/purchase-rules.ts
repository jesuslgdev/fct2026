...
export function assertPurchaseCanBeEdited(status: PurchaseStatus): void {
  if (!canEditPurchase(status)) {
    throw new PurchaseBusinessRuleError(
      'Only purchases in Pending status can be edited.',
    );
  }
}
...
export function assertPurchaseCanBeDeleted(status: PurchaseStatus): void {
  if (!canDeletePurchase(status)) {
    throw new PurchaseBusinessRuleError(
      'Only purchases in Pending status can be deleted.',
    );
  }
}
...
export function calculatePurchaseTotals(lines: PurchaseLineInput[]): PurchaseTotals {
  ...
  const totals = lines.reduce(
    (acc, line) => {
      const lineTotals = calculatePurchaseLineTotals(line);
      return {
        subtotal: roundCurrency(acc.subtotal + lineTotals.subtotal),
        vatTotal: roundCurrency(acc.vatTotal + lineTotals.vatAmount),
        total: roundCurrency(acc.total + lineTotals.total),
      };
    },
    {
      subtotal: 0,
...
export function normalizePurchaseQueryParams(
  ...
  const normalized: PurchaseQueryParams = {
    ...
    pageSize:
      Number.isInteger(params.pageSize) && params.pageSize > 0
        ? params.pageSize
        : DEFAULT_PURCHASES_PAGE_SIZE,
    sort: params.sort ?? DEFAULT_PURCHASES_SORT,
  };

  if (normalized.supplierId !== undefined) {
    assertPositiveInteger(normalized.supplierId, 'supplierId');
  }

  const createdFrom = parseIsoDate(normalized.createdFrom, 'createdFrom');
  const createdTo = parseIsoDate(normalized.createdTo, 'createdTo');
